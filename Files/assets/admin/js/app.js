    ...
    for(var i = 0; i < input.files.length ; i++){
      ...
      if( i != 0){
        ...
        reader.onload = function (e) {
            $('.imageField').append(` <div class="thumb">
                <div class="avatar-preview">
                  <div class="profilePicPreview" style="background-image: url('${e.target.result}')"></div>
                </div> 
                <div class="avatar-edit">
                    <input type="file" name="image[]" class="profilePicUpload" id="${e.target.result}" accept=".png, .jpg, .jpeg" multiple>
                    <label class="bg--danger removeBtn"><i class="la la-trash"></i></label>
                    <label for="${e.target.result}" class="bg--primary"><i class="la la-pencil"></i></label>
                </div>
            </div>`);
        }
